...
final class QuestionViewController: UIViewController {
    ...
    @IBAction func multipleAnswerButtonPressed(_ sender: Any) {
        for (multiplesSwitch, answer) in zip(multipleSwitches, currentAnswers) {
    ...
    @IBAction func rangedAnswerButtonPressed(_ sender: Any) {
        let index = lrintf(rangedSlider.value) //эта функция округляет флоат до целого числа
...
private extension QuestionViewController {
    func updateUI() {
        ...
        title = "Dопрос № \(questionIndex + 1) из \(questions.count)"
